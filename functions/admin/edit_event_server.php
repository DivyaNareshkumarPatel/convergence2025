...
if (in_array($icon_fileExt, $allowedExtensions)) {
    if (in_array($flyer_fileExt, $allowedExtensions)) {
        if ($icon_fileError === 0 and $flyer_fileError === 0) {
            ...
            $sql = "UPDATE event SET event_name=?, event_icon_path=?, event_tagline=?, flyer_path=?, description=?, rules=?, evaluation_criteria=?, date=?, time=?, venue=?, faculty_coordinator=?, student_coordinator=?, volunteer=?, time_slot=? WHERE department_name=? AND event_name=?";
            $stmt = mysqli_prepare($conn, $sql);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssssssssssssss", $param_event_field_name, $param_event_icon_path, $param_event_tagline, $param_flyer_path, $param_description, $param_rules, $param_evaluation_criteria, $param_date, $param_time, $param_venue, $param_faculty_coordinator, $param_student_coordinator, $param_volunteer, $param_time_slot, $param_department_name, $param_event_name);
                $param_event_field_name = $field_name;
